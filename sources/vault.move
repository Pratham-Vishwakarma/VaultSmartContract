module the_vault::vault {
    ...
    public entry fun claim_tokens(account: &signer, vault_address: address) acquires Vault, VaultSignerCapability {
        ...
        let vault_signer_cap = &borrow_global<VaultSignerCapability>(vault_address).cap;
    ...
    #[view]
    public fun get_allocation(vault_address: address, addr: address): u64 acquires Vault {
        let vault = borrow_global<Vault>(vault_address);
        if (table::contains(&vault.allocations, addr)) {
            *table::borrow(&vault.allocations, addr)
        } else {
            0
        }
    }
